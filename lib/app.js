...
async function estatico(request, url) {
  ...
  try {
    ...
    //
    // Sin esto un <video> no puede saltar dentro del archivo: el
    // navegador declara seekable [0,0] y solo puede reproducir lo que
    // ya bajo. Con la portada arrastrada por el scroll eso significa
    // que el video no se mueve, o que hay que esperar a que baje
    // entero antes de poder tocarlo.
    //
    // En Netlify esto lo resuelve la CDN, pero el servidor de trabajo
    // tiene que comportarse igual o lo que se prueba no es lo que se
    // publica.
    ...
    if (m && info.size > 0) {
      ...
      if (inicio === null) {
        ...
        const cuantos = Math.min(fin || 0, info.size);
        inicio = info.size - cuantos;
        fin = info.size - 1;
      } else if (fin === null || fin >= info.size) {
        fin = info.size - 1;
      }

      if (!Number.isFinite(inicio) || inicio > fin || inicio < 0) {
        return responder(request, 416, 'text/plain; charset=utf-8', 'rango invalido',
          { 'content-range': 'bytes */' + info.size, 'accept-ranges': 'bytes' });
      }

      const flujo = Readable.toWeb(createReadStream(destino, { start: inicio, end: fin }));
      return responder(request, 206, tipo, flujo, {
    ...
    return responder(request, 200, tipo, await readFile(destino), {
      'cache-control': cache,
      'accept-ranges': 'bytes'
    });
  } catch {

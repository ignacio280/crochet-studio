...
function sslPara(cadena) {
  let host = '';
  try { host = new URL(cadena).hostname; } catch (e) {}
  if (host === 'localhost' || host === '127.0.0.1') return false;
  if (process.env.DB_SSL_SIN_VERIFICAR === '1') return { rejectUnauthorized: false };
  const esInterno = !host.includes('.');
  return { rejectUnauthorized: !esInterno };
}
...
async function pgClient() {
  ...
  const pool = new Pool({
    ...
    // El certificado se verifica solo cuando vale la pena.
    //
    // Render entrega la base por un nombre interno sin puntos (dpg-xxxx) que
    // solo existe dentro de su red privada, y con certificado autofirmado:
    // ahi verificar falla siempre y no protege de nada, porque el trafico
    // nunca sale a internet. Con un servidor externo (con dominio) si se
    // verifica, que es donde importa.
